...
```bash
docker build -t tmd .
# 如果宿主机挂载目录属主不是 uid/gid 1000，可以调整：
# docker build --build-arg PUID=$(id -u) --build-arg PGID=$(id -g) -t tmd .
```

**首次运行（交互式生成配置）**

`--conf` 流程需要从标准输入读取，所以第一次必须带 `-it`：

```bash
docker run -it --rm \
  -v tmd_config:/home/tmd/.tmd2 \
  -v $(pwd)/downloads:/downloads \
  tmd --conf
```

`storage path` 建议直接填 `/downloads`（对应上面挂载的宿主机 `./downloads` 目录）。
...
docker run --rm \
  ...
  tmd --user elonmusk --skip-invalid
```

也可以用 `docker-compose.yml`：

```bash
docker compose run --rm tmd --conf                       # 首次配置
docker compose run --rm tmd --user elonmusk --skip-invalid   # 日常调用
```

> `~/.tmd2`（配置、sqlite 数据库、日志、失败推文 dump）和媒体下载目录分别挂载为两个卷，容器可随时删除重建而不丢数据。为创建符号链接，容器内进程运行在拥有目标目录写权限的非 root 用户下即可，无需特权模式。
...
tmd --skip-invalid         // 跳过无效/无法解析的 user_id、screen_name 或 list_id，仅下载其余有效目标
```
